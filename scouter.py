...
def nmap1():
	...
	try:
		...
		if a != "":
			...
			results = os.popen("nmap -sV -sC -p " + a + " " + host + "| head -n -3 | sed '1d;2d;3d;4d;'| tr '|' '-'| tr -d '_'").read()
			results = results.rstrip("\r\n")
			print(Style.BRIGHT + Fore.CYAN + '[+] Non-default TCP Scan Report' + '\n' + '\n' + Style.RESET_ALL + results + '\n')
			return 
     		else:
			currentDT = datetime.datetime.now()
			zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "			
...
def nmapS():
	...
	zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "
	try:
		a=os.popen("nmap -sC -sV " + host + "| head -n -3 | sed '1d;2d;3d;4d;'| tr '|' '-'| tr -d '_'").read()
		a=a.rstrip("\r\n")
		print(Style.BRIGHT + Fore.CYAN + '[+] Default TCP Scan Report' + '\n' + '\n' + Style.RESET_ALL + a + '\n')
		return
	except:
...
def udp():
	...
	try:	
		...
		if b != "":
			...
			result = os.popen("nmap -sU -sV -sC -p " + b + " " + host + "| head -n -3| sed '1d;2d;3d;4d;'| tr '|' '-'| tr -d '_'").read()
			result = result.rstrip("\r\n")
			print(Style.BRIGHT + Fore.CYAN + '[+] UDP Scan Report' + '\n' + '\n' + Style.RESET_ALL + result + '\n')
			return
     		else:
			currentDT = datetime.datetime.now()
			zeTime = "[" + currentDT.strftime("%H:%M:%S") + "] "			
...
if __name__=='__main__':
	...
	print(Style.BRIGHT + Fore.MAGENTA + '  / ___/_________  __  __/ /____  _____     v.9000+')
	...
	print('\n')
	initialScans = 'Starting initial scans...'
	print('\n' + Style.BRIGHT + Fore.YELLOW + zeTime + initialScans + Style.RESET_ALL + '\n')
	pcop = Process(target = cop)
	pcop.start()	
	pnmap1 = Process(target = nmap1)
	pnmap1.start()
